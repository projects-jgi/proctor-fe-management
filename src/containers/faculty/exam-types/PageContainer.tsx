  ...
  return (
    <div className='my-8 space-y-4'>
        ...
        <section>
            ...
            {types.data && types.data.map((type: ExamTypeResponse, index: number) => (
                <Card key={index} className='mt-4'>
                    <CardHeader className='gap-6'>
                        <CardTitle>
                            {type.name}
                            <Badge className='ms-2' variant={type.is_private ? 'secondary' : 'default'}>{type.is_private ? 'Private' : 'Public'}</Badge>
                        </CardTitle>
                        <CardAction>
