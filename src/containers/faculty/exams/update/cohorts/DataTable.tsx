...
interface DataTableProps<TData, TValue> {
  data: TData[];
  mapped_cohort_exam_data: TData[];
  exam_id: number;
}

export function DataTable<TData, TValue>({
  data,
  mapped_cohort_exam_data,
  exam_id,
  ...
  useEffect(() => {
    if (tableType === "edit") {
      ...
      setCurrentData(
        Object.values(mapped_cohort_exam_data).map(
          (cohort: any) => cohort.cohort,
        ),
      );
    }
  }, [tableType]);
  ...
  useEffect(() => {
    if (mapped_cohort_exam_data) {
      setRowSelection(
        Object.fromEntries(
          mapped_cohort_exam_data.map((mapping: any) => [
            mapping.cohort.id,
            true,
          ]),
        ),
  ...
  async function handleSave() {
    toast.promise(
      () => {
        const cohort_ids = Object.keys(rowSelection).map(Number);

        return create_cohort_exam_mappings({
          exam_id,
          cohort_ids,
        }).then((response) => {
          if (response.status) {
            queryClient.invalidateQueries({
              queryKey: ["faculty", "exams", exam_id, "cohorts"],
            });
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <Input
              ...
              value={
                (table.getColumn("cohort_name")?.getFilterValue() as string) ??
                ""
              }
              onChange={(event) =>
                table
                  .getColumn("cohort_name")
            ...
            <CardAction className="flex items-center gap-4">
              ...
              {tableType == "edit" &&
                ...
                Object.keys(rowSelection).length > 0 && (
                  <Button onClick={handleSave}>Map Cohorts</Button>
                )}
            </CardAction>
          </div>
        </CardHeader>
