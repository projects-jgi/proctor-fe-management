  ...
  return (
    <div className="my-8 space-y-4">
            ...
            <div className="flex items-center gap-4 md:gap-8 text-sm flex-wrap">
              ...
              <p className="text-muted dark:text-muted-foreground">
                ...
                {violations.dataUpdatedAt &&
                  new Date(
                    Number(violations.dataUpdatedAt)
                  ).toLocaleTimeString()}
              </p>
            </div>
            <div className="flex items-center gap-4 md:gap-8">
              <Select
                value={refreshInterval ? String(refreshInterval) : undefined}
              ...
              >
                ...
                <SelectContent>
                  <SelectGroup>
                    <SelectItem value="5000" defaultChecked>
                      5s
                    </SelectItem>
                    <SelectItem value="10000" defaultChecked>
                      10s
                    </SelectItem>
                    <SelectItem value="30000" defaultChecked>
                      30s
                    </SelectItem>
                    <SelectItem value="60000" defaultChecked>
                      1m
                    </SelectItem>
                    <SelectItem value="300000" defaultChecked>
                      5m
                    </SelectItem>
                  </SelectGroup>
                </SelectContent>
      ...
      <section className="mt-8">
        ...
        {violations.isError && (
          <div className="text-center text-muted-foreground text-sm">
            Error loading violations: {violations.error?.message}
          </div>
        )}
        {violations.data && violations.data.length === 0 && (
          <div className="text-center text-muted-foreground text-sm">
            No violations found for this exam.
          </div>
        )}
        ...
        <Accordion type="multiple">
          {violations.data &&
            violations.data.map(
              (violation: ViolationResponse, index: number) => {
                if (
                  violation.student_user.name
                    .toLowerCase()
                        ...
                        <CardHeader>
                          <AccordionTrigger>
                            <CardTitle>
                              ...
                              <Badge
                                variant={
                                  violation.ended_at ? "success" : "warning"
                                }
                              >
                                {violation.ended_at ? "Submitted" : "Ongoing"}
                              </Badge>
                            </CardTitle>
                          </AccordionTrigger>
                          <CardDescription>
                        ...
                        <AccordionContent>
                          <CardContent>
                            {violation.violations.map(
                              (
                                violationDetail: StudentExamAttemptViolation,
                                index: number
                              ) => (
                                <div
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-xs text-muted-foreground underline">
                                        <a
                                          href={violationDetail.reference_url}
                                          target="_blank"
                                        >
                                          {violationDetail.reference_url}
                                        </a>
                                      </p>
                                    </div>
                                  </div>
